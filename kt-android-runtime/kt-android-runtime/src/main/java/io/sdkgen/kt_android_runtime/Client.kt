    ...
    suspend fun makeRequest(functionName: String, bodyArgs: JsonObject?, timeoutMillis: Long = defaultTimeoutMillis): InternalResposne = suspendCoroutine { continuation ->
        ...
        try {
            val body = JsonObject().apply {
                ...
                addProperty("staging", useStaging)
            }

            val request = Request.Builder()
                .url("https://$baseUrl${if (useStaging) "-staging" else ""}/$functionName")
            ...
            try {
                ...
                if (!responseBody.get("ok").asBoolean) {
                    ...
                    continuation.resume(InternalResposne(null, responseBody.getAsJsonObject("result")))
                }
                response.close()
            } catch (e: Exception) {
                httpTimer.cancel()
